//
//  MyRecruitmentDownloadedViewController.m
//  dww
//
...
@interface MyRecruitmentDownloadedViewController ()<UITableViewDataSource, UITableViewDelegate>
@property (strong, nonatomic) IBOutlet UITableView *tableView;
@property (strong, nonatomic) NSMutableArray *tableViewDataSource;
@end

@implementation MyRecruitmentDownloadedViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.navigationItem.title = @"近期谁下载过我的简历";
    // tableView
...
- (void)requestListData
{
    ...
    exprame = [NSMutableDictionary dictionaryWithObjectsAndKeys:
               ...
               @"999",@"size",
               nil];
    [HaoConnect request:@"job_company_down_v/list" params:exprame httpMethod:METHOD_GET onCompletion:^(HaoResult *result) {
        [MBProgressHUD hideAllHUDsForView:self.view animated:YES];
        if (result.isResultsOK) {
            if (!result.results || [result.results count] < 1) {
                [self.view configBlankPage:EaseBlankPageTypeNoPersonDownLoadResume hasData:NO hasError:NO reloadButtonBlock:nil customButtonBlock:nil];
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    cell.cellType = RecruitmentCellTypeDownload;
