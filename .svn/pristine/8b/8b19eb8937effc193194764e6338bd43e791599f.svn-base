...
- (PYSearchSuggestionViewController *)searchSuggestionVC
{
    if (!_searchSuggestionVC) {
        ...
        searchSuggestionVC.view.frame = CGRectMake(0, self.navigationController.navigationBar.translucent ? 64 : 0, self.view.py_width, self.view.py_height);
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    ...
    if (self.searchSuggestionVC.view.hidden) { // 搜索建议隐藏
        // 清空搜索建议
        self.searchSuggestions = nil;
        [self.view sendSubviewToBack:self.searchSuggestionVC.view];
    } else
    {
        // 放在最上层
